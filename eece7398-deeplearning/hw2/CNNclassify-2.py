...
class seeNet (object):
    def __init__ (self, isTrain=1):
        ...
        'fourth_layer': tf.get_variable(name="w4", shape=[24*24*128, nHNeurons[0]]), 
        ...
        layer_one = tf.nn.dropout(layer_one, 0.5)
        ...
        layer_two = tf.nn.dropout(layer_two, 0.5)
        ...
        layer_three = tf.nn.dropout(layer_three, 0.5)

        layer_three = tf.reshape(layer_three, [-1, 24*24*128])
...
if sys.argv[1] == "train":
    ...
    classify.train(xTrain, yTrain, xTest, yTest, maxSteps=40)
